...
To add:
gseaplot2(y_inhib_dmso, geneSetID = 1:3, pvalue_table = TRUE,
          color = c("#E495A5", "#86B875", "#7DB0DD"))

enrichplot::upsetplot(y_inhib_dmso)

enrichplot::heatplot(y_inhib_dmso, showCategory = 15)

y_inhib_dmso <- setReadable(y_inhib_dmso, 'org.Hs.eg.db', 'ENTREZID')

enrichplot::cnetplot(y_inhib_dmso_wnames)

dotplot(y_inhib_dmso)
...
```{r}
res_t6li_c11wt <- DESeq2::results(dds, contrast=c("Sample_Group","T6_C11_LI","BJAB_C11_WT"), alpha = 0.05)

res_t6wt_c11wt <- DESeq2::results(dds, contrast=c("Sample_Group","T6_C11_WT","BJAB_C11_WT"), alpha = 0.05)
```

```{r}
a<-data.frame(gene = rownames(res_t6li_c11wt), res_t6li_c11wt,
...
b<-data.frame(gene = rownames(res_t6wt_c11wt), res_t6wt_c11wt,
stringsAsFactors = F) %>% na.omit() %>% dplyr::select("log2FoldChange", "padj")%>% dplyr::mutate(minuslogp = -log10(padj), lfcsign = log2FoldChange/abs(log2FoldChange), signlogp = lfcsign*minuslogp) 
...
pheatmap(subset(vsdMat_t6_ko, rownames(vsdMat_t6_ko) %in% sig_list_combined), cluster_rows=TRUE, show_rownames=TRUE,cluster_cols=FALSE, annotation_col=df_t6_ko, scale = "row", fontsize = 25, fontsize_col = 25, fontsize_row = 20, angle_col = 90, main="Genes with |LFC|>1 & padj<0.05 in at least one of these comparisons:\n1) C11MT vs C11WT. 2) T6KO C11MT vs T6KO C11WT", legend_breaks = )
dev.off()
```
```{r}
pdf("Heatmap_siglist_c11li_c11wt.pdf", width=18, height = 24)
pheatmap(subset(vsdMat_t6_ko, rownames(vsdMat_t6_ko) %in% sig_genelist_c11li_c11wt), cluster_rows=TRUE, show_rownames=TRUE,cluster_cols=FALSE, annotation_col=df_t6_ko, scale = "row", fontsize = 25, fontsize_col = 25, fontsize_row = 20, angle_col = 90, main="Genes with |LFC|>1 & padj<0.05 in C11MT vs C11WT")
dev.off()
```
```{r}
pdf("Heatmap_siglist_t6li_t6wt.pdf", width=21, height = 18)
pheatmap(subset(vsdMat_t6_ko, rownames(vsdMat_t6_ko) %in% sig_genelist_t6li_t6wt), cluster_rows=TRUE, show_rownames=TRUE,cluster_cols=FALSE, annotation_col=df_t6_ko, scale = "row", fontsize = 25, fontsize_col = 25, fontsize_row = 20, angle_col = 90, main="Genes with |LFC|>1 & padj<0.05 in T6KO C11MT vs T6KO C11WT")
dev.off()
```
...
pheatmap(subset(vsdMat_inhib_dmso, rownames(vsdMat_inhib_dmso) %in% heatmap_list_inhib_dmso), cluster_rows=TRUE, show_rownames=TRUE,cluster_cols=TRUE, annotation_col=df_inhib_dmso, scale = "row", fontsize = 10, fontsize_col = 10, fontsize_row = 10, angle_col = 90, main="Inhib vs DMSO Selected Genes")
dev.off()
```
...
pheatmap(subset(vsdMat_inhib_dmso, rownames(vsdMat_inhib_dmso) %in% sig_genelist_inhib_dmso), cluster_rows=TRUE, show_rownames=TRUE,cluster_cols=TRUE, annotation_col=df_inhib_dmso, scale = "row", fontsize = 10, fontsize_col = 10, fontsize_row = 10, angle_col = 90, main="Inhib vs DMSO Selected Genes")
dev.off()
```
...
union_signature_genesymbols <- unique(c(NFkB_Up_BCR_paper_genesymbols,
                                      ...
                                      NFkB_Up_all_OCILy3_Ly10_genesymbols))
```
...
subset(t6li_t6wt_list_signatures$gene, t6li_t6wt_list_signatures$gene %in% NFkB_Up_all_OCILy3_Ly10_genesymbols)
...
subset(c11li_c11wt_list_signatures$gene, c11li_c11wt_list_signatures$gene %in% NFkB_Up_all_OCILy3_Ly10_genesymbols)
```
...
png("signatures_boxplot_Union.png", width=600, height=800)
...
```{r}
signatures_list <- c("NFkB-1", "NFkB-2", "NFkB-4", "NFkB-5", "NFkB-9", "NFkB-10")
...
```{r}
png("sig_enrich_merged_scatter.png", width=1200, height=800)
...
```{r}
png("sig_enrich_nfkb_scatter.png", width=1200, height=800)
ggplot(subset(sig_enrich_merged, sig_enrich_merged$ID %in% signatures_list) , aes(x=NES.x, y=NES.y))+
  geom_point()+
  geom_label_repel(aes(label=ID), size=3, color="black", fontface="bold",  min.segment.length = unit(0, "lines"), max.overlaps = 20)+
  xlim(2,3)+
  ylim(2,3)
dev.off()
```
